                      ...
                      const SensorProcessorBase::GeneralParameters& generalSensorProcessorParameters)
{
    ...
    {
        RCLCPP_ERROR(nodeHandle_->get_logger(),
                     "Could not configure input source %s because no sensor_processor was given.",
                     inputSourceName.c_str());
    }
...
std::string Input::getSubscribedTopic() const
{
    ...
    // FIXME:
    return rclcpp::expand_topic_or_service_name(parameters.topic_, nodeHandle_->get_name(),
