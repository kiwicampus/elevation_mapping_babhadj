...
bool StereoSensorProcessor::readParameters(std::string& inputSourceName)
{
    ...
    nodeHandle_->get_parameter(inputSourceName + ".sensor_processor.depth_to_disparity_factor",
                                             ...
                                             Eigen::VectorXf& variances)
{
    ...
    // Preparations for#include <pcl/common/transforms.h> robot rotation Jacobian (J_q) to minimize computation for
    ...
    for (unsigned int i = 0; i < pointCloud->size(); ++i)
    {
        ...
        double disparity = sensorParameters_.at("depth_to_disparity_factor") /
...
int StereoSensorProcessor::getJ(int index) { return indices_[index] % originalWidth_; }
